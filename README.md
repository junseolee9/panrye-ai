...
평가: LLM 저지 4메트릭(RAGAS 방법론 준용, 케이스당 1콜 한국어 루브릭 — `eval/judge.py`) + 결정적 루브릭(도메인 정확도, 법조문·판례 인용률, 허위 사건번호 스캔). 테스트셋 30케이스, 6개 도메인 × 5.
RAGAS 자체를 쓰지 않은 이유: 무료 티어(비OpenAI) 저지 + 한국어 조합에서 verdict 파싱이 자주 깨져 0/NaN이 나오고, 메트릭당 4콜이 rate limit으로 생성 쿼터까지 잠식했다. 단일 콜 + JSON 강제 + 저지/생성 쿼터 분리(Gemini/Groq)로 재설계.

### 측정에서 나온 개선 이력 · 로드맵

- ✅ **도메인 라우팅 70%→90%**: 첫 풀런에서 오분류 9건이 전부 "민사"로 쏠리며 도메인 필터 검색을 붕괴시킴 → 공백 무시 키워드 매칭 + 렉시콘 보강 + LLM 폴백 8b→70b 승격으로 해결
- ⬜ **Context Precision (0.53)**: 리랭커 점수 컷오프 도입 — 현재는 무조건 top-5를 전달해 무관 판례가 섞임. cross-encoder 점수 하한선으로 노이즈 차단 예정
- ⬜ **허위 사건번호 (30건 중 3건)**: 생성 후 결정적 검증 — 답변 속 사건번호가 컨텍스트에 없으면 마스킹하는 post-validator
- ⬜ **Context Recall (0.45)**: 코퍼스 확장(하급심·최신 판례)과 함께, 모범답안의 절차 안내(노동청 진정 등)는 판례 본문에 존재하지 않아 구조적으로 낮게 측정되는 문제 분리 측정 필요
...
- 무료 티어 LLM 쿼터(Groq 10만 토큰/일) 기준 30케이스 풀 평가 = 하루 1회 분량 — 평가 러너에 체크포인트·재개·폴백 재시도 내장
