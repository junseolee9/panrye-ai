...
- **데이터 품질이 모델보다 먼저다.** 검색 품질 문제의 진짜 원인은 임베딩이 아니라 메타데이터 없는 코퍼스 76%였다.
- **평가는 파이프라인만큼 설계가 필요하다.** 저지 LLM의 파싱 신뢰성, 쿼터 간섭, 폴백 오염 — 측정 시스템이 무너지면 개선 방향 자체를 잘못 잡는다.
- **무료 티어는 설계 제약이다.** 모델별 쿼터 분리(생성 70b / 경량 태스크 8b / 저지 gpt-oss), 체크포인트, 폴백 체인 — 비용 0원을 유지하는 건 공짜가 아니었다.

## 한계

- 코퍼스는 법제처 공개 판례 일부(5,667건) — 최신·하급심 판결 누락 가능
- CPU 추론 기준 응답 15~25초 (임베딩·리랭킹·요약 전부 로컬 모델)
