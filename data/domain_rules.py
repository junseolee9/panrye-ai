"""
판례 도메인 분류 규칙 (수집 단계 라벨링).
agents/classifier.py 라벨 체계와 동일: 형사/민사/가족법/행정/노동/부동산/기타
우선순위: 참조법령 > 사건종류명 > 본문 키워드.
"""
...
def classify_precedent(
    ...
    """법령명/사건종류명/본문 키워드 점수 합산으로 도메인 결정."""
    joined_laws = " ".join(statutes)
    combined = f"{case_name} {text[:1000]}"
    scores: dict[str, int] = {}

    for law, domain in LAW_TO_DOMAIN:
        if law in joined_laws:
            scores[domain] = scores.get(domain, 0) + WEIGHT_LAW

    if "민법" in joined_laws:
        is_family = any(h in combined or h in joined_laws for h in MINBEOP_FAMILY_HINTS)
